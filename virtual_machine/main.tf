...
# https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs/data-sources/images_image_v2
data "openstack_images_image_v2" "volume_image" {
  for_each    = var.virtual_machine.volumes != null ? { for idx, vol in var.virtual_machine.volumes : idx => vol if try(length(vol.image), 0) > 0 } : {}
...
resource "openstack_blockstorage_volume_v3" "volume" {
  count = var.virtual_machine.volumes != null ? length(var.virtual_machine.volumes) : 0
  image_id = try(
    data.openstack_images_image_v2.volume_image[tostring(count.index)].id,
    null
  )
...
resource "openstack_compute_instance_v2" "virtual_machine" {
  ...
  image_id        = var.virtual_machine.image != null ? data.openstack_images_image_v2.image.id : null

  # Include block_device only if volumes are provided
  dynamic "block_device" {
    for_each = var.virtual_machine.volumes != null && length(var.virtual_machine.volumes) > 0 ? { for idx, volume in var.virtual_machine.volumes : idx => volume } : {}
